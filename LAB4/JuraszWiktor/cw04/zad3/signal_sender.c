...
int main(int argc, char *argv[]){
	...
	sigset_t ss;
	if(sigaddset(&ss, SIGRTMIN+SIGUSR1) == -1) print_error(errno, "error in sigaddset 1");
	if(sigaddset(&ss, SIGRTMIN+SIGUSR2) == -1) print_error(errno, "error in sigaddset 2");

	if(sigaction(SIGRTMIN+SIGUSR1, sigusr1_sigaction, NULL) == -1) print_error(errno, "errno in sigaction(SIGUER1)");
	if(sigaction(SIGRTMIN+SIGUSR2, sigusr2_sigaction, NULL) == -1) print_error(errno, "errno in sigaction(SIGUER2)");
	...
	for(i = 0; i< process_number; i++)
		if(kill(pid, SIGRTMIN+SIGUSR1) != 0) print_error(errno, "error in SIGUSR1 kill");

	if(kill(pid, SIGRTMIN+SIGUSR2) != 0) print_error(errno, "error in SIGUSR2 kill");
	while(!sigusr2_recived){
	       	pause();//tutaj zostajei
	}
	if(sigusr1_counter == process_number) printf("OK");
