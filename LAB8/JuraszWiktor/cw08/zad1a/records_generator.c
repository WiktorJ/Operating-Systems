...
struct record {
    int id;
    char text[REC_SIZE - sizeof(int)];
};
char* randomString(size_t length, char *returnValue) {
...
int main(int argc, char *argv[]){
	...
	*argv++;
	...
	for(i = 0; i < records_number; i++){
        ...
        r -> id = i;
        randomString(REC_SIZE - sizeof(int),r -> text);
        if(write(handle, r, sizeof(struct record)+1) != sizeof(struct record)+1) print_error(errno, "blad zapisu");
	}
//	int n;
//	char buf[size];
