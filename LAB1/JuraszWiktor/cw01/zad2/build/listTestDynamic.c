...
int main(){
	...
	clock_t currReal = startTime;
 	void* handler = dlopen("liblistShared.so",RTLD_LAZY);
	char *error;
    	if(!handler)    	{
    	     fprintf(stderr, "%s\n", dlerror());
       	 exit(EXIT_FAILURE);
	...
	void (*addContact)(struct List*, struct Node*);
	void (*deleteContactByEmail)(struct List **, char *);
	struct Node* (*searchByEmail)(struct List *, char *);
	void (*mergeSort)(struct Node **);
	struct Node* (*merge)(struct Node *, struct Node *);
	void (*frontBackSplit)(struct Node *, struct Node **, struct Node **);
	void (*sortBySurname)(struct List *);
	...
	addContact=dlsym(handler,"addContact");
	deleteContactByEmail=dlsym(handler,"deleteContactByEmail");
	searchByEmail=dlsym(handler,"searchByEmail");
	mergeSort=dlsym(handler,"mergeSort");
	merge=dlsym(handler,"merge");
	frontBackSplit=dlsym(handler,"frontBackSplit");
	sortBySurname=dlsym(handler,"sortBySurname");
	printNode=dlsym(handler,"printNode");
	printList=dlsym(handler,"printList");
